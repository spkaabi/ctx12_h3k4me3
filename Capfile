...
task :upload_data, :roles => group_name do
	puts "copy below and run directly on server under screen"    
	puts "rsync -e 'ssh -i /home/spkaabi/ec2/spkaabi.pem' -vzP data/* ubuntu@ec2-46-137-55-227.eu-west-1.compute.amazonaws.com:#{mount_point}" 
   end
...
task :run_macs, :roles => group_name do
  ...
  ctx_treatment = "#{mount_point}/CMN050_s_1_export_sorted_nodups.bam"
  ctx_control = "#{mount_point}/CMN051_s_3_export_sorted_nodups.bam"

  fbs_treatment = "#{mount_point}/CMN048_s_8_export_sorted_nodups.bam"
  fbs_control = "#{mount_point}/CMN049_s_4_export_sorted_nodups.bam"

  bmp_treatment = "#{mount_point}/CMN046_s_7_export_sorted_nodups.bam"
  bmp_control = "#{mount_point}/CMN047_s_8_export_sorted_nodups.bam"
  ...
  bws.each {|bw|
    pvalues.each { |pvalue|
      ...
      dir = "#{mount_point}/macs_#{bw}_#{pvalue}_ctx"
      run "rm -Rf #{dir}"
      run "mkdir #{dir}"

      macs_cmd =  "macs --treatment #{ctx_treatment} --control #{ctx_control} --name #{group_name} --format BAM --gsize #{genome} --bw #{bw} --pvalue #{pvalue}"
      run "cd #{dir} && #{macs_cmd}"

      dir = "#{mount_point}/macs_#{bw}_#{pvalue}_fbs"
      run "rm -Rf #{dir}"
      run "mkdir #{dir}"

      macs_cmd =  "macs --treatment #{fbs_treatment} --control #{fbs_control} --name #{group_name} --format BAM --gsize #{genome} --bw #{bw} --pvalue #{pvalue}"
      run "cd #{dir} && #{macs_cmd}"

      dir = "#{mount_point}/macs_#{bw}_#{pvalue}_bmp"
      run "rm -Rf #{dir}"
      run "mkdir #{dir}"

      macs_cmd =  "macs --treatment #{bmp_treatment} --control #{bmp_control} --name #{group_name} --format BAM --gsize #{genome} --bw #{bw} --pvalue #{pvalue}"
      run "cd #{dir} && #{macs_cmd}"
...
#      macs_cmd =  "macs --treatment #{ctx_treatment} --control #{ctx_control} --name #{group_name} --format BAM --gsize #{genome} --call-subpeaks  --wig --bw #{bw} --pvalue #{pvalue}"
#      run "cd #{dir} && #{macs_cmd}"
#
#      macs_cmd =  "macs --treatment #{fbs_treatment} --control #{fbs_control} --name #{group_name} --format BAM --gsize #{genome} --call-subpeaks  --wig --bw #{bw} --pvalue #{pvalue}"
#      run "cd #{dir} && #{macs_cmd}"
#
#      macs_cmd =  "macs --treatment #{bmp_treatment} --control #{bmp_control} --name #{group_name} --format BAM --gsize #{genome} --call-subpeaks  --wig --bw #{bw} --pvalue #{pvalue}"
#      run "cd #{dir} && #{macs_cmd}"
#
